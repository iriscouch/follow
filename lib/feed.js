...
Feed.prototype.query = function query_feed() {
  ...
  feed_request.created_at = now
  feed_request.id = function() { return feed_id }

  feed_request.on('response', function() {
    self.log.debug('Remove feed from agent pool: ' + feed_request.id())
  ...
  function response_timed_out() {
    self.log.debug('Feed response timed out: ' + feed_request.id())
    timed_out = true
    return self.retry()
  }
  ...
  function on_feed_response(er, resp, body) {
    ...
    if(timed_out) {
      self.log.debug('Ignoring late response: ' + feed_request.id());
      return destroy_response(resp);
    }

    if(er) {
      self.log.debug('Request error ' + feed_request.id() + ': ' + er.stack);
      destroy_response(resp);
      return self.retry();
    }

    if(resp.statusCode !== 200) {
      self.log.debug('Bad changes response' + feed_request.id() + ': ' + resp.statusCode);
      destroy_response(resp);
      return self.retry();
    }

    self.log.debug('Good response: ' + feed_request.id());
    self.retry_delay = INITIAL_RETRY_DELAY;

    self.emit('response', resp);
    return self.prep(feed_request);
  }
}
