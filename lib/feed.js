...
Feed.prototype.on_couch_data = function on_couch_data(change) {
  ...
  if(self.dead)
    return self.log.debug('Dead feed skipping change: ' + change.seq)
  self.on_change(change)

  // Again, the change handler may have killed this feed.
  if(self.dead)
    return self.log.debug('Dead feed skipping wait')
  self.wait()
}
